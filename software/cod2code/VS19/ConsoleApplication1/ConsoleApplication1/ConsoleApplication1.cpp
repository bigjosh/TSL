...
class Top_finder {
	...
	typedef std::pair<int, int> key_value_pair;


	struct cmp_value {
		bool operator() (key_value_pair a, key_value_pair b) const {
			return a.second > b.second;
		}
	};


	set<key_value_pair, cmp_value> get_pairs_sorted_by_keys() {
	...
	// Used for counting
	// value, count
	map<int, int> keys_values_map;

	public:
	
	void increment(int key) {

		if ( keys_values_map.find(key) == keys_values_map.end() ) {
			keys_values_map.insert(key_value_pair(key, 0));
		}


		keys_values_map.find(key)->second++;
	}

	// I am sorry for these ugly iterators. I do not know C++ and did not want to install BOOST to get foreach() which is waht I really want. 



	// Print the sorted list of keys and counts in markdown style

	void print_sorted_key_count_md() {

		set<key_value_pair, cmp_value> sorted_keys = get_pairs_sorted_by_keys();

		for( auto const& kv_pair : sorted_keys ) {
	...
	vector<int> keys_decending() {

		// Load the pairs into a set that is sorted by vales decending

		set<key_value_pair, cmp_value> sorted_keys = get_pairs_sorted_by_keys();

		// Append the keys into a vector in Decending order so we can Return the vector of decending sorted keys only 

		vector<int> ret;

		for (auto const& pair : sorted_keys) {
...
void emit_code_for_lcd_steps(lcd_reg_state initial_lcd_reg_state, lcd_reg_state sequence_of_lcd_reg_states[], int sequence_count) {
	...
	lcd_reg_state regs_now;		// Used to hold current register values

	// First we do a trial pass to find which 
	// LCD registers are accessed most often and values are stored most often 


	Top_finder lcd_reg_access_counter;
	Top_finder lcd_load_values_access_counter;
	...
	for (int i = 0; i < sequence_count; i++) {
		...
		for (uint8_t r = 0; r < LCD_REG_COUNT; r++) {
			...
			if (regs_now.regs[r] != regs_next.regs[r]) {		// Dont count if the register did not change
				...
				lcd_reg_access_counter.increment(r);
				lcd_load_values_access_counter.increment(regs_next.regs[r]);
	...
	auto sorted_lcd_regsisters = lcd_reg_access_counter.keys_decending();
